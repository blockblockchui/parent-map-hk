#!/usr/bin/env python3
"""
Google Sheet 自動座標查核工具（gspread 版本）
使用 gspread 庫，更穩定的 Service Account 支援
"""
...
def process_sheet():
    ...
    print("="*70)
    print("Google Sheet 自動座標查核工具（gspread）")
    print("="*70)
    ...
    try:
        ...
        print("  2. Sheet ID 正確")
        print("  3. 已將 Service Account (place-search@parent-map-hk.iam.gserviceaccount.com)")
        print("     分享給該 Sheet 作為編輯者")
        return
    ...
    try:
        ...
        col_place_name = headers.index('Name')  # 實際列名是 Name
        col_address = headers.index('Address')  # 實際列名是 Address
        # district 列可能不存在
        try:
            col_district = headers.index('district')
        except ValueError:
            col_district = -1  # 標記為不存在
    ...
    # 篩選未查核的行（如果 FORCE_ALL=true 則檢查所有行）
    force_all = os.environ.get('FORCE_ALL', '').lower() == 'true'
    
    if force_all:
        unchecked_indices = list(range(2, len(all_values) + 1))  # 所有行
        print(f"   ⚠️  強制模式：檢查所有 {len(unchecked_indices)} 行")
    else:
        unchecked_indices = []
        for i, row in enumerate(all_values[1:], start=2):  # 從第2行開始（1-based）
    ...
    # 處理
    print(f"\n🔍 開始查核 {len(unchecked_indices)} 個地點...\n")
    ...
    for idx, row_num in enumerate(unchecked_indices, 1):
        row = all_values[row_num - 1]  # 轉換為 0-based 索引
        ...
        time.sleep(0.5)  # 避免 Places API 速率限制
